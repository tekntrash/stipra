...
class CustomRenderRadialGauge extends RenderBox {
  ...
  void _paintAxis(
    ...
    if (axis.segments != null) {
      for (var segment in axis.segments!) {
        _paintTexts(axis, segment, context, widgetOffset);
  ...
  void _paintTexts(RadialGaugeAxis axis, RadialGaugeSegment segment,
      PaintingContext context, Offset widgetOffset) {
    final minAngle = segment.minAngle;
    final maxAngle = segment.maxAngle;
    final minValue = segment.minValue;
    final textStyle = TextStyle(
      color: Colors.black,
      fontSize: 12,
    );
    ...
    //context.canvas.translate(-size.width / 2, -size.height / 2);
    //context.canvas.rotate(pi / 2);
    //context.canvas.translate(-size.width / 2, -size.height);
    final outerRadius = innerRadius + renderWidth;
    final outerCirclePoint = Offset(
        (outerRadius * 1) *
                cos((minAngle + (maxAngle - minAngle) / 2) * degrees2Radians) +
            size.height / 2,
        (outerRadius * 1.0) *
                sin((minAngle + (maxAngle - minAngle) / 2) * degrees2Radians) +
            size.width / 2);
    ...
    if (minValue <= 0) {
      offset = Offset(
        -textPainter.width * 1.15,
        -(textPainter.height / 2),
      );
    } else if (minValue <= 20) {
      offset = Offset(
        -textPainter.width * 1.15,
        -(textPainter.height * 0.75),
      );
    } else if (minValue <= 40) {
      offset = Offset(
        -(textPainter.width / 2),
        -(textPainter.height * 1.25),
      );
    } else if (minValue <= 60) {
      offset = Offset(
        textPainter.width * 0.15,
        -(textPainter.height * 0.75),
      );
    } else if (minValue <= 80) {
      offset = Offset(
    ...
    context.canvas.save();
    context.canvas.translate(outerCirclePoint.dx, outerCirclePoint.dy);
    context.canvas.rotate(pi / 2);
